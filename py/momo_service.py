...
from momo_db import db_connect, delete_latest_incomplete_snapshot, get_dashboard_data, get_dashboard_data_page, get_latest_decrease_alerts, get_today_workspace, import_api_snapshot, now_bj, query_words, record_incomplete_snapshot, setup_schema, get_local_word_index, get_current_records_by_words
from momo_config import backend_host, backend_port, sync_cooldown_minutes, token_path
from momo_api_client import assert_success, build_study_count_lookup, count_first_response, count_new_words, count_pending_new_words, count_review_words, enrich_items_with_study_count, fetch_all_records, post_json, read_token, today_items

try:
    from lemminflect import getAllLemmas
except Exception:  # optional dependency; article inflection filtering reports a clear error when requested
    getAllLemmas = None
...
def sync_api_to_sqlite_browser(*, token_file: Path, db_path: Path, log_func=print) -> dict:
    ...
    This sync path fetches Maimemo API data and writes SQLite directly.
    It only uses SQLite as the local persistence layer.
    """
    ...
    if alerts:
        for a in alerts:
            log_func("提醒：" + str(a.get("message") or a))
    return {
        "success": True,
...
def read_sync_cooldown_minutes(root_dir: Path) -> int:
    """Read browser-triggered sync cooldown from config/app.json."""
...
def latest_snapshot_status(conn: sqlite3.Connection) -> dict[str, object] | None:
    row = conn.execute(
        """
        SELECT snapshot_time, COALESCE(data_complete, 1) AS data_complete, COALESCE(api_success, 1) AS api_success
        FROM snapshots
        ORDER BY snapshot_time DESC
        LIMIT 1
        """
...
def extract_article_words(text: str, *, min_len: int = 2, skip_stopwords: bool = True) -> list[dict[str, object]]:
    ...
    for idx, m in enumerate(re.finditer(r"[A-Za-z][A-Za-z'\u2019-]*", text or "")):
        token = normalize_article_word(m.group(0))
    ...
    return [
        {"word": w, "count": counts[w], "firstOrder": first_order[w]}
        for w in sorted(counts, key=lambda x: first_order[x])
    ]
...
def merge_notepad_content(content: str, words: list[str], chapter: str = "") -> tuple[str, list[str]]:
    ...
    for line in (content or "").splitlines():
        ...
        if not stripped or stripped.startswith("#"):
            continue
        w = normalize_article_word(stripped.split()[0])
        if w:
            existing.add(w)
    cleaned = []
    seen = set()
    for w in words:
        word = normalize_article_word(w)
...
class MomoRequestHandler(SimpleHTTPRequestHandler):
    server_version = "MomoSQLiteHTTP/1.0"

    def translate_path(self, path: str) -> str:
        parsed = urlparse(path)
        path = unquote(parsed.path)
        rel = path.lstrip("/") or "history_dashboard.html"
        # Keep static serving inside root.
        safe = Path(self.server.root_dir, *[p for p in rel.split("/") if p and p not in (".", "..")])
        return str(safe)
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/sync":
            self.handle_sync()
            return
        if parsed.path.startswith("/api/notepads") or parsed.path in ("/api/article/analyze", "/api/study/advance", "/api/study/add-words"):
    ...
    def handle_write_api(self, parsed) -> None:
        try:
            ...
            if path == "/api/notepads/create":
                ...
                notepad = create_notepad(token, title=title, brief=str(body.get("brief") or ""), content=str(body.get("content") or ""), tags=[str(x) for x in tags], status=str(body.get("status") or "PUBLISHED"))
            ...
            if path == "/api/notepads/add-words":
                ...
                words = [normalize_article_word(str(x)) for x in words]
                ...
                if notepad_id:
                    ...
                    notepad = create_notepad(token, title=title, brief=str(body.get("brief") or "从总览页文章分析生成"), content=content, tags=[], status="PUBLISHED")
            ...
            if path == "/api/article/analyze":
                ...
                for item in words:
                    w = str(item["word"])
                    in_local, local_match = find_form_match(w, local_index, local_form_index, use_forms=exclude_inflections)
                    in_notepad, notepad_match = find_form_match(w, notepad_words, notepad_form_index, use_forms=exclude_inflections) if notepad_id else (False, "")
                    if exclude_inflections and (local_match or notepad_match) and w not in (local_match, notepad_match):
                    ...
                    if missing:
                        results.append({
                            ...
                            "forms": sorted(article_word_forms(w)) if exclude_inflections else [w],
                        })
                self.send_json({"success": True, "uniqueCount": len(words), "missingCount": len(results), "missingWords": results, "mode": mode, "excludeInflections": exclude_inflections, "excludedByInflectionCount": excluded_by_inflection})
                return
            if path == "/api/study/advance":
                ...
                if not voc_ids and words:
                    for part in chunked([normalize_article_word(str(x)) for x in words if str(x).strip()], 1000):
                        voc_ids.extend([vid for vid in (api_voc_id(v) for v in query_vocabulary(token, spellings=part)) if vid])
                voc_ids = [str(x) for x in voc_ids if str(x).strip()]
                if not voc_ids:
            ...
            service_log(f"POST 接口异常 {path}: {exc}")
    ...
    def handle_sync(self) -> None:
        ...
        if cooldown_min > 0:
            try:
                ...
                if latest_status and not latest_status.get("complete"):
                    pass  # bypass cooldown for incomplete
                elif latest:
                    ...
                    if diff_min is not None and diff_min < cooldown_min:
                        ...
                        if not force:
                            msg = f"距离上次快照仅 {diff_min} 分钟（阈值: {cooldown_min} 分钟），是否仍要联网同步？"
                            self.send_json({
                                "success": True, "needConfirm": True, "reason": "cooldown",
                                "message": msg, "latestSnapshotTime": latest,
                                "minutesSinceLatest": diff_min, "cooldownMinutes": cooldown_min,
                            })
                            return
            except Exception as exc:
                print(f"读取上次快照时间失败: {exc}", flush=True)
        ...
        def _do_sync():
            try:
                result = sync_api_to_sqlite_browser(
        ...
        try:
            while True:
                try:
                    ...
                    _stream(json.dumps({"ping": True}))
                    continue
                if msg is None:
                    break
                _stream(json.dumps({"log": msg}))
            sync_thread.join(timeout=10)

            if "result" in result_holder:
                final = result_holder["result"]
                _stream(json.dumps({"result": final}))
            else:
                _stream(json.dumps({"error": result_holder.get("error", "未知错误")}))
    ...
    def handle_api(self, parsed) -> None:
        ...
        heavy_paths = {"/api/dashboard-data", "/api/dashboard-data-page", "/api/words"}
        ...
        try:
            ...
            with db_connect(self.server.db_path) as conn:
                service_log(f"API 数据库连接+打开 {parsed.path} 耗时 {(time.perf_counter() - t0) * 1000:.1f}ms")
                # schema 已在服务启动时检查。读接口不再每次 setup_schema，避免并发请求反复 CREATE INDEX 造成 Termux 上长时间等待。
                ...
                if parsed.path == "/api/dashboard-data":
                    t0 = time.perf_counter()
                    payload = get_dashboard_data(conn)
                    service_log(f"API /api/dashboard-data 构建完成 days={len(payload.get('days', []))} 耗时 {(time.perf_counter() - t0) * 1000:.1f}ms，总耗时 {(time.perf_counter() - request_start) * 1000:.1f}ms")
                    self.send_json(payload)
                    return
                if parsed.path == "/api/dashboard-data-page":
                    offset = int(query.get("offset") or 0)
                    limit = int(query.get("limit") or 3)
                    include_meta = str(query.get("includeMeta") or "").lower() in ("1", "true", "yes")
                    order = str(query.get("order") or "asc")
                    workspace_date = str(query.get("date") or "").strip() or None
                    workspace_only = str(query.get("workspaceOnly") or "").lower() in ("1", "true", "yes")
                    t0 = time.perf_counter()
                    if workspace_only:
                        payload = {"success": True, "todayWorkspace": get_today_workspace(conn, day_key=workspace_date)}
                        service_log(f"API /api/dashboard-data-page workspaceOnly date={workspace_date} 耗时 {(time.perf_counter() - t0) * 1000:.1f}ms")
                    else:
                        payload = get_dashboard_data_page(conn, offset=offset, limit=limit, include_meta=include_meta, order=order, workspace_date=workspace_date)
                        service_log(
                            f"API /api/dashboard-data-page offset={offset} limit={limit} loaded={payload.get('loaded')} total={payload.get('total')} "
                            f"db={payload.get('timings', {}).get('total_db_ms')}ms 构建耗时={(time.perf_counter() - t0) * 1000:.1f}ms 总耗时={(time.perf_counter() - request_start) * 1000:.1f}ms"
                        )
                    self.send_json(payload)
                    return
                if parsed.path == "/api/words":
                    ...
                    service_log(
                        ...
                        f"items={len(payload.get('items') or [])} 耗时={(time.perf_counter() - t0) * 1000:.1f}ms "
                        f"明细={payload.get('timings', {})}"
                    )
                    self.send_json(payload)
                    return
                if parsed.path == "/api/records-by-words":
                    words = [x.strip() for x in str(query.get("words") or "").split(",") if x.strip()]
                ...
                if parsed.path == "/api/days":
                    rows = conn.execute("SELECT DISTINCT study_day_key FROM snapshots WHERE COALESCE(data_complete, 1)=1 AND COALESCE(api_success, 1)=1 ORDER BY study_day_key").fetchall()
                    self.send_json({"success": True, "days": [r["study_day_key"] for r in rows]})
                    return
                self.send_error_json(HTTPStatus.NOT_FOUND, f"未知接口: {parsed.path}")
        except ClientDisconnected:
...
class MomoHTTPServer(ThreadingHTTPServer):
    ...
    def runtime_health(self) -> dict[str, object]:
        ...
        return {
            ...
            "serviceVersion": "20260514_busy_reuse_guard",
